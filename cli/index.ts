...
YargsParser.command(
	...
	.options('options', {
		describe: ', use dot notation, e.g. options.nativeScrollbars'
	}).argv;
...
async function serve(port: number, pathToSpec: string, options: Options = {}) {
	...
	const server = createServer((request, response) => {
		...
		if (request.url === '/redoc.standalone.js') {
			respondWithGzip(createReadStream(join(BUNDLES_DIR, 'redoc.standalone.js'), 'utf8'), request, response, {
...
async function getPageHTML(
	spec: any,
	pathToSpec: string,
	{ ssr, cdn, title, disableGoogleFont, templateFileName, templateOptions, redocOptions = {} }: Options
) {
	let html;
	let css;
	let state;
